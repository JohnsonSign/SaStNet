...
class TSNDataSet(data.Dataset):
    ...
    def _get_test_indices(self, video_list):
        if self.dense_sample:  # i3d dense sample
            ...
            # print('777777777') #使用test_center_crop 和 test_three_crop 测试something数据集都是走这里
            if len(video_list) > self.num_segments + self.new_length - 1:
                tick = (len(video_list) - self.new_length + 1) / float(self.num_segments)
                start_list = np.linspace(0, tick - 1, num=4, dtype=int)
                start_index = start_list.tolist()[0]              
                # offsets = np.array([int(tick / 2.0 + tick * x) for x in range(self.num_segments)])
                # offsets = np.array([int(tick * x) for x in range(self.num_segments)])
                offsets = np.array([int(tick * x + start_index) for x in range(self.num_segments)])
            else:
                offsets = np.zeros((self.num_segments,))
            return offsets + 1
            # tick = (len(video_list) - self.new_length + 1) / float(self.num_segments)
    ...
    def __getitem__(self, index):
        ...
        if('something' in self.dataset) or ('hmdb' in self.dataset) or ('UAVHuman' in self.dataset):
            decode_boo = False
            video_name = record.path.split('/')[-1]
            rgb_path = os.path.join(self.root_path,video_name)
            video_list = sorted(os.listdir(rgb_path))
        ...
        #print('segment_indices: ',segment_indices) [ 2 12 22 32 39 45 50 64]
        return self.get(record,video_list, segment_indices,decode_boo,rgb_path)

    def get(self, record,video_list, indices,decode_boo,rgb_path=None):
        images = list()
        for seg_ind in indices:
            ...
            for i in range(0,self.new_length,1):
                if(decode_boo):
                    ...
                    seg_imgs = self._load_image(rgb_path,p)
                ...
                if((len(video_list)-self.new_length*1+1)>=8):
                    ...
                    if p < (len(video_list)):
                        p += 1
        #print('images size: ',len(images),images[0].size,images[0].mode) # 40, (150,100)

        process_data, record_label = self.transform((images,record.label))

        #print('process_data: ',process_data.size()) # torch.Size([120, 224, 224])
