...
def accuracy(output, target, topk=(1, )):
    ...
    _, pred = output.topk(maxk, 1, True, True) # 按行返回5个排序值(从大到小)索引
    pred = pred.t()
    correct = pred.eq(target.view(1, -1).expand_as(pred)) # 相等为1,不等为0
    res = []
    for k in topk:
...
def accuracy_mixup(output, target, target_shuffle, lam, topk=(1, 5)):
    maxk = max(topk)
    batch_size = target.size(0)

    _, pred = output.topk(maxk,1,True,True) #[8,5]
    pred = pred.t() #[5,8]
    correct = (1-lam) * pred.eq(target.view(1, -1).expand_as(pred)) + lam * pred.eq(target_shuffle.view(1, -1).expand_as(pred)) #[5,8]
    res = []
    for k in topk:
        correct_k = correct[:k].contiguous().view(-1).float().sum(0)
        res.append(correct_k.mul_(100.0 / batch_size))
